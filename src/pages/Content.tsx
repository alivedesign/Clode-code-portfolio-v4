...
function YouTubeCard({ videoId }: { videoId: string }) {
  ...
  return (
    <div className={`relative w-full rounded-[24px] overflow-hidden cursor-pointer transition-all duration-300 ${playing ? "opacity-100 scale-100" : "opacity-65 hover:opacity-100 hover:scale-[1.02]"}`} style={{ paddingBottom: "56.25%" }}>
      {playing ? (
        <iframe
